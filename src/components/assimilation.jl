...
struct AssimilationC{NSB,KD,YDE,NC} <: AbstractAssimilationC
...
assimilation!(J_DE::Matrix{Float64}, p::AssimilationC, D::Vector{Float64}, V::Vector{Float64}) = begin
    n_substrates = size(D,1)
    n_consumers  = size(V,1)
...
uptake!(J_D::Matrix{Float64}, p::AssimilationC, D::Vector{Float64}, V::Vector{Float64}) = begin
    n_substrates = size(D,1)
    n_consumers  = size(V,1)
    n_minerals   = size(M(p),1)
    k2p_D        = 180.0*60*60*ones(n_substrates, n_consumers)
...
assimilation_production!(J_DE_CO2::Matrix{Float64}, p::AssimilationC, D::Vector{Float64}, V::Vector{Float64}) = begin
    n_substrates = size(D,1)
    n_consumers  = size(V,1)
    n_minerals   = size(M(p),1)
    k2p_D        = 180.0*60*60*ones(n_substrates, n_consumers)
